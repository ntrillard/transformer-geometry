...
def main():
    ...
    pd.DataFrame([results]).to_csv(OUT / "defense_mitigation__gemma-3-1b.csv", index=False)
    df.to_csv(OUT / "cowtip_robustness__gemma-3-1b.csv", index=False)
    print(f"\nSaved -> {OUT}/cowtip_robustness__gemma-3-1b.csv, "
          f"{OUT}/defense_mitigation__gemma-3-1b.csv   ({time.perf_counter()-t0:.0f}s)")
