...
def main():
    ...
    df.to_csv(OUT / "boundary_margins__gemma-3-1b.csv", index=False)
    ...
    cdf.to_csv(OUT / "cone_theta__gemma-3-1b.csv", index=False)

    print(f"Saved {OUT/'boundary_margins__gemma-3-1b.csv'} and "
          f"{OUT/'cone_theta__gemma-3-1b.csv'}  ({time.perf_counter()-t0:.0f}s)")
