...
def load_model(model_id, dtype="fp16", quantize=None, attn_implementation=None):
...
def competitor_controls(u, s, W, target_id, delta):
    """Return target rank after residual-latitude rotations that keep both
    ||v|| and the target score v@s fixed.

    Decomposition: v0 = gamma*s + rho*r (r ⟂ s).  We rotate only the residual
    component in the plane spanned by r and a random q (q ⟂ s, q ⟂ r):
        v(eps) = gamma*s + rho*(cos(eps)*r + sin(eps)*q)
    This preserves norm and target score by construction, isolating the
    competitor-direction variable.
    """
    tau = tangent_direction(u, s)
    v0 = rotate_toward(u, tau, delta)
    target_score = float(v0 @ s)
    ...
    # decompose v0 = gamma*s + rho*r, with r ⟂ s
    gamma = target_score
    rho_vec = v0 - gamma * s
    ...
    if rho > 1e-8:
        ...
        r = _random_in_nullspace(v0, s, seed=0)
        if r is None:
            r = normalize(np.random.randn(len(v0)) - (np.random.randn(len(v0)) @ s) * s)
        rho = 0.0

    # random direction q in the residual plane (orthogonal to both s and r)
    q_rand = _random_in_nullspace(r, s, seed=0)
    if q_rand is None:
        q_rand = r

    # project competitor direction onto the residual plane
    comp_perp = comp_dir - (comp_dir @ s) * s
    if np.linalg.norm(comp_perp) > 1e-8:
        toward = normalize(comp_perp)
    else:
        toward = r
    away = -toward

    eps = math.radians(8)  # off-arc latitude angle
    results = {}
    for name, q in [("random", q_rand), ("toward_blocker", toward), ("away_blocker", away)]:
        v = gamma * s + rho * (math.cos(eps) * r + math.sin(eps) * q)
        ...
        results[name] = {
            ...
            "norm_error": float(abs(np.linalg.norm(v) - 1.0)),
            "score_error": float(abs(v @ s - target_score)),
        }
    return results
...
def _batched_block(u, W_t, Wn_t, tid_idx, max_angle, seed):
    ...
    # Wrong-target control: guarantee wrong_target != target (no self-draws).
    K = len(tid_idx)
    ...
    # 4. residual-latitude competitor controls (batched): keep ||v|| and v@s fixed.
    # Decompose target-tangent endpoint v0 = gamma*s + rho*r (r ⟂ s), then rotate
    # only the residual component in the residual plane.
    v0 = cosd * un[None, :] + sind * TAU            # (K,d)
    gamma = (v0 * S).sum(1, keepdim=True)           # (K,1)
    rho_vec = v0 - gamma * S                        # (K,d)
    rho = rho_vec.norm(dim=-1, keepdim=True)        # (K,1)
    R = _bn(rho_vec)                                # (K,d)
    # Q random in nullspace of S and R (equivalently S and v0)
    B0 = torch.randn(len(tid_idx), d, device=u.device)
    B0 = B0 - (B0 * S).sum(1, keepdim=True) * S - (B0 * R).sum(1, keepdim=True) * R
    Q_rand = _bn(B0)
    ...
    comp_perp = comp_dir - (comp_dir * S).sum(1, keepdim=True) * S
    toward = _bn(comp_perp)
    ok_t = comp_perp.norm(dim=-1) > 1e-8
    toward = torch.where(ok_t[:, None], toward, Q_rand)

    eps = math.radians(8)
    res = {}
    for name, q in [("random", Q_rand), ("toward_blocker", toward), ("away_blocker", -toward)]:
        v = gamma * S + rho * (math.cos(eps) * R + math.sin(eps) * q)
        v = _bn(v)
        vL = v @ WT
        ...
        for ctx in contexts:
            for l in layers:
                ...
                for tid in target_ids:
                    ...
                    wrong_tid = rng.choice([x for x in target_ids if x != tid])
