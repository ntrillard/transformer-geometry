...
def main():
    ...
    print("(steps: S = settled start state, 1-8 = one 4-deg re-aimed step toward the target family)")
    print()
    for l in spheres:
        steps = walk(int(l))
        print(f"sphere {l}:")
        for i, (_, t, c) in enumerate(steps):
            tok_s = t.strip() if t.strip() else repr(t)
            label = "S" if i == 0 else str(i)
            print(f"   {label:>2}  {tok_s:<16} ({c})")
        print()
