#!/usr/bin/env python3
"""gen_blend.py - readout-time BLEND of unsteered and steered states.
Built from gen_steer.py's graft machinery, but instead of injecting the
fully-steered state at a switch step, inject

    blended = normalize((1-lam)*v + lam*s) * ||v||

where v is the natural final hidden state (unsteered) and s is the fully
steered state (rotated to the target's calibrated rank-1 angle). lam is
the BLEND percentage (BLEND_PCT, 0..1): lam=0 -> pure unsteered, lam=1
-> pure graft.

Steering is readout-time only: the hook on the final RMSNorm overwrites
its last-token output, which is exactly what the LM head reads. Held
states are never fed back through the layers.

Env:  BLEND_PCT=<0..1> (default 0.5)
      TRACE=1          (print effective angle per switch)

Run:  HF_TOKEN=<tok> python3 gen_blend.py [model] [prompt] [w1,w2,..]
"""
import math
import os
import sys
...
def main():
    t0 = time.time()
    print(f'\nBlend generation | {MODEL} | prompt={PROMPT!r} | words={WORDS} '
          f'| BLEND_PCT={BLEND_PCT} | ntok={NTOK}')
    ...
    def blend_states(vv, ss, lam):
        """renormalized linear blend (1-lam)*v + lam*s, same norm as v."""
        b = (1 - lam) * vv + lam * ss
        n = vv.norm()
        return b / b.norm() * n
    ...
    for step in range(NTOK):
        ...
        if step in switch_at:
            ...
            s = rot_to_angle(v, word_ids[w], th)      # fully steered state
            b = blend_states(v, s, BLEND_PCT)         # blended state
            L = forward(ids, inj_p=b)
            plant_word = w
            plant_tid = word_ids[w]
            plant_until = step + 1 + ANTI
            if TRACE:
                ang = math.degrees(math.acos(
                    float((v / v.norm()) @ (b / b.norm()))))
                print(f'      switch@{step} -> {w}:{tok.decode([word_ids[w]])!r} '
                      f'lam={BLEND_PCT} full={th:.0f}deg eff={ang:.1f}deg')
    ...
    print(f'\n===== STEERED blend lam={BLEND_PCT} ({", ".join(WORDS)}) =====')
    print(f'{PROMPT} {txt}')
