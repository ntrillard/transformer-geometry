...
def gen_with_detector(model, tok, head, prompt_ids, pit, max_new=MAX_NEW,
                      temperature=None, top_p=None, stop_on_repeat=None):
    ids = list(prompt_ids)
    consec = 0
...
def main():
    ...
    def load(dtype="fp16", quant=None):
        return M.load_model(MODEL, dtype=dtype, quantize=quant)
    ...
    for cfg in [("fp16", None), ("bf16", None), ("fp16", "int8")]:
        dtype, quant = cfg
        m, tk = load(dtype, quant)
        head_t = torch.as_tensor(m.lm_head.weight.detach().float().cpu().numpy(),
                                device=m.device)
        for label, t in [("pit", pit), ("control", ctl)]:
            s, _ = self_score(m, tk, t, head_t)
            keep, cosl, _ = permanence(m, tk, t, head_t)
    ...
    for label, kw in [("greedy", dict()), ("temp=1.0 top-p=.9", dict(temperature=1.0, top_p=0.9)),
        ...
        print(f"  {label:16s} mean trailing-pit tokens/run = {np.mean(runs):.1f}")
    # template
    ...
    print(f"  normal-generation harm: median truncation = {np.median(harm):.0f} tokens "
          f"(detector vs no-detector over {len(norms)} prompts)")
    ...
    pd.DataFrame([results]).to_csv(OUT / "defense_mitigation__Qwen2-0.5B.csv", index=False)
    df.to_csv(OUT / "cowtip_robustness__Qwen2-0.5B.csv", index=False)
    print(f"\nSaved -> {OUT}/cowtip_robustness__Qwen2-0.5B.csv, "
          f"{OUT}/defense_mitigation__Qwen2-0.5B.csv   ({time.perf_counter()-t0:.0f}s)")
